...
- **Web sysop console** — SYSOP login dashboard for message management, user admin, and live session monitoring
...
The sysop console login form expects a callsign and password. On a packaged
install, `deploy/install.sh` creates the initial `SYSOP` account and prints the
generated password. It also saves the same bootstrap note at:

```bash
/root/pybulletin-initial-sysop.txt
```

To view it later on the node:

```bash
sudo cat /root/pybulletin-initial-sysop.txt
```

After login, change the password from the sysop console. The browser receives a
temporary session token from `/api/auth/login`; operators do not need to create
...
API authentication uses the temporary session token returned by
`/api/auth/login`. Browser requests handle this automatically after the sysop
logs in. API clients should send it as `Authorization: Bearer <token>`; every
request to `/api/*` except `/api/health` requires an authenticated session.
...
The sysop console uses normal user credentials. The initial `SYSOP` password is
generated by `deploy/install.sh` and written to
`/root/pybulletin-initial-sysop.txt`.
